<div
	class="transform hover:scale-105 transition duration-500 ease-in-out bg-white rounded-lg shadow-lg"
>
	<a href="https://www.google.com" target="_blank">
		<img
			src="https://images.unsplash.com/photo-1600054800747-be294a6a0d26?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1053&q=80"
			alt=""
			class="rounded-lg"
		/>
	</a>
	<!-- <div class="p-4">
		<h2 class="font-bold mb-2 text-xl text-purple-800">Card with an image</h2>
		<p class="text-purple-700 mb-2">This is a little bit better of a card!</p>
	</div> -->
</div>
